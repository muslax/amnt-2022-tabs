  ...
  return (
    <div className="p-5">
      ...
      <h1 className="text-lg font-bold mb-6">{ desa
        ? `Data hasil hutan Desa ${desa}`
        : 'Data hasil hutan seluruh desa'
      }</h1>
          ...
          <table className="w-full text-[12px] whitespace-nowrap">
            <tbody>
              <tr className="bg-gray-100 border-b font-semibold">
                ...
                <td className="p-2 border-l">jumlah</td>
                <td className="p-2 border-l">satuan</td>
            ...
            {daftar().map((res:any, i:number) => (
              <tr key={res._id} className="border-b last:border-none">
                ...
                <td className="p-2 border-l">{res.jumlah}</td>
                <td className="p-2 border-l">{res.satuan}</td>
...
export async function getStaticProps() {
  ...
  const dfItem = await db.collection('hutan').aggregate([
    { $match: {}},
    { $lookup : {
      from: "responden",
      localField: "_idr",
